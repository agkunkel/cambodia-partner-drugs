...
running_model_final_west <- function(scenario, which_proph,fitness_sens_ana, fixed_inputs,variable_inputs_all) {
...
# loooping through all variables
for (i in 1:dim(variable_inputs_all)[1]) {
	...
	variable_inputs["f_mp"] <- variable_inputs["f_m"]*variable_inputs["f_p"]
	...
	# Adding MQ res
	# to mosquitoes
	yinit2$E_qm <- pars_vec$mdr_2000*yinit2$E_qn
	yinit2$E_qn <- (1-pars_vec$mdr_2000)*yinit2$E_qn
	yinit2$I_qm <- pars_vec$mdr_2000*yinit2$I_qn
	yinit2$I_qn <- (1-pars_vec$mdr_2000)*yinit2$I_qn
	# to humans
	...
	# running model for 3rd time period
	...
	results$I_a <- with(results,{ I_han + I_ham + I_hap +
	 ...
	 results$prop_trans_asym <- with(results,{ b*I_a/(I_s + b*I_a) })	# transmission from asymptoamtic
	results$prev_pcr <- with(results,{(I_s+I_a)/2700000})  # PCR prevalence in full human population
	results$prev_mosq <- with(results,{(I_qn + I_qm + I_qp + I_qmp)/Nm}) 
	...
	# resistance - among new casses only
		results$I_q_inc <- with(results,{I_qn + pars$f_m*I_qm + pars$f_mp*I_qp + pars$f_mp*I_qmp })
	results$prev_pfmdr <- with(results,{(pars$f_m*I_qm + pars$f_mp*I_qmp)/(I_q_inc)}) # of all inf
	 results$prev_pfpm <-  with(results,{(pars$f_p*I_qp + pars$f_mp*I_qmp)/(I_q_inc)}) # of all inf
	 results$prev_pfb <-  with(results,{(pars$f_mp*I_qmp)/(I_q_inc)}) # of all inf
	results$prev_mres <- (1-pars$s_m)*results$prev_pfmdr # TES only include symptomatic I'd think
...
write.csv(prev_pcr_out, file= paste(scenario, which_proph,"west_prev_pcr.csv",sep="_"))
...
write.csv(rep_case_out, file= paste(scenario, which_proph,"west_rep_case.csv",sep="_"))
...
write.csv(prev_pfmdr_out, file= paste(scenario, which_proph,"west_prev_pfmdr.csv",sep="_"))
...
write.csv(prev_pfpm_out, file= paste(scenario, which_proph,"west_prev_pfpm.csv",sep="_"))
...
write.csv(prev_pfb_out, file= paste(scenario, which_proph,"west_prev_pfb.csv",sep="_"))
...
write.csv(mres_prop_out, file= paste(scenario, which_proph,"west_mres_prop.csv",sep="_"))
...
write.csv(pres_prop_out, file= paste(scenario, which_proph,"west_pres_prop.csv",sep="_"))
...
write.csv(bres_prop_out, file= paste(scenario, which_proph,"west_bres_prop.csv",sep="_"))

}

# ###########################################################################################
############################## PLOTTING LINES AND POINTS - OLD #########################
###########################################################################################
...
if(which_proph=="NONE" & scenario=="short_baseline") {
  ...
  pdf(file= paste(which_proph,"_west.pdf",sep=""))

	# to indicate which drug was used when
  drug_mat=as.data.frame(matrix(nrow=3,ncol=3))
  colnames(drug_mat) <- c('Start','End','Drug')
  drug_mat$Start <- as.numeric(c(2000, 2008, 2017))
drug_mat$End <- as.numeric(c(2008, 2017, Inf))
...
cases <- c(1660, 5160, 940,2460 ,1820,4800, 900,2160, 700,2200, 1160,3080, 640,1580, 780,1820, 600,780, 220,340, 361,834, 343,792, 224,518, 400,926, 307,708, 167,386)
...
p <- ggplot(data = results_tmp) + geom_path(aes(x=year,y= value,group=variable)) + geom_point(aes(x=year,y=cases),col='orangered', size=3) +  ylab('Monthly cases') + ylim(0,7000) +  theme_bw() +
	geom_rect(data=drug_mat, aes(xmin = Start, xmax = End,   ymin = -Inf, ymax = Inf, fill = Drug), alpha = 0.2 ) + 
  geom_text(aes(x = Start, y = 6000, label = Drug), data = drug_mat,  size = 3, vjust = 0, hjust = 0, nudge_x = 0) +
...
# West
gen_slide_N <-c(2811,2943,4586,4114)
gen_slide_pf <- c(45,11,8,1)
fg_slide_prop <- gen_slide_pf/gen_slide_N
...
p <- ggplot(data = pcr_results) + geom_path(aes(x=year,y= value,group=variable)) + geom_point(aes(x=year,y= fg_pcr_prop),col='orangered',size=3) + ylab('PCR Prevalence') + 
	geom_rect(data=drug_mat, aes(xmin = Start, xmax = End,   ymin = -Inf, ymax = Inf, fill = Drug), alpha = 0.2 ) + 
  geom_text(aes(x = Start, y = 0.08, label = Drug), data = drug_mat,  size = 3, vjust = 0, hjust = 0, nudge_x = 0) +
   ylim(0,0.1) +  theme_bw() +
   scale_fill_manual(values = c("gray80", "gray100"))  + scale_x_continuous(expand = c(0, 0), limits=c(2000,2020))
print(p)

# # ############ Plot pfmdr1 copy numbers - west
...
dates <- c(2008.5, 2009.5, 2010.5, 2011.5, 2012.5, 2013.5, 2011.5, 2012.5, 2013.5, 2008.5, 
	...
	2017.5, 2014.5, 2015.5, 2016.5, 2018.5, 2015.5, 2016.5, 2018.5, 2015.5, 2016.5, 2015.5, 
	2016.5, 2017.5,
	2019.5, 2019.5, 2019.5) 

N_tested <- c(38, 91, 60, 110, 49, 64, 81, 72, 32, 21, 58, 37, 52, 163, 71, 57, 8, 42, 100, 
	15, 67, 34, 20, 53, 94, 22, 39, 40, 273, 224, 47, 8, 3, 56, 53, 48, 29, 43, 51, 40, 63, 53, 
	4, 96, 33, 200,
	22, 13, 16) 
...
# # ############ Plot pfpm2 copy numbers - west

pfpm_results <- melt(prev_pfpm_df,id.vars="year")

dates <- c(2003, 2005, 2007,2009, 2009, 2011, 2011, 2011, 2013, 2013, 2013, 2013, 2013, 
...
N_tested <- c(60, 117, 43, 68, 30, 21, 73, 34, 39, 23, 39, 22, 17, 16, 57, 57, 67, 34,
	  20, 53, 94, 22, 39, 14, 56, 53, 3, 48, 29, 43, 51, 41, 63, 4, 94, 129, 35, 200,
	  22, 13, 16) 

N_multicopies <-c(1, 8, 12, 19, 0, 13, 28, 3, 31, 13, 31, 1, 8, 3, 52, 52, 1, 1, 8, 
...
dates <- c(2001.5, 2002.5, 2002.5, 2002.5, 2003.5, 2003.5, 2003.5, 2004.5, 2004.5, 
		2004.5, 2011.5, 2014.5, 2015.5, 2017.5, 2010.5, 2016.5, 2017.5, 2015.5, 2014.5,
		 2016.5, 2003, 2007,
		 2019.5, 2019.5, 2019.5, 2018.5) 

N_tested <- c(50, 70, 67, 29, 91, 90, 52, 81, 81, 85, 28, 138, 59, 51, 45, 46, 55, 
		60, 59, 60, 162, 143,
		12, 22, 16, 11) 
	
N_LTF <-c(2, 10, 4, 1, 3, 2, 4, 8, 6, 0, 0, 0, 0, 0, 5, 0, 0, 0, 0, 0, 4, 27,
		0, 0, 2, 0) 
...
dates <- c(2010.5, 2012.5, 2010.5, 2010.5, 2011.5, 2012.5, 2015.5, 2013.5, 2011.5,
		 2013.5, 2014.5, 2013.5, 2013.5, 2008.5, 2009.5, 2008.5, 2009.5, 2013.5, 2003)

N_tested <- c(28, 39, 11, 56, 42, 22, 53, 17, 59, 16, 40, 81, 63, 47, 37, 74, 
		60, 89, 162)
	
N_LTF <-c(7, 12, 2, 6, 7, 0, 29, 1, 2, 1, 25, 37, 10, 5, 3, 1, 0, 42, 4)
