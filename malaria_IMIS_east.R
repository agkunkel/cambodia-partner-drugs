#### This file contains code needed to fit the parameteres for Eastern Cambodia

options(error=browser)

# This function calculates the overall log likelihood of the model based on a set or sets of input parameters
...
llikelihood <- function(inpars) {
	...
	mdr_2000 = 0.1 # proportion with multiple pfmdr1 copies in the year 2010 
							# (midleading varaible name - MQ resistance in East in introduced in 2010 due to limited prior data)
	...
	end_time_init2 = (2010-1995)*366 # EAST
	...
	# Conditions to initalize the model (1995-2000)
		yinit <- c(S_m = 0.99, E_mn = 0, E_mm = 0, E_mp = 0, E_mb = 0,
			 ...
			 S_h = pars_vec$N_init-100, E_hn = 0, E_hm = 0, E_hp = 0, E_hb = 0, 
	...
	names(yinit2) <- ynames
		...
		# # Run the model for second time period (2000-2010, East)
	...
	# 2010 outputs will be inputs for next run
	...
	# Introducing MQ resistance (2010 in Eastern Cambodia because few data before that)
	
	# introducing to mosquitoes
	yinit3$E_mm <- pars_vec$mdr_2000*yinit3$E_mn
	yinit3$E_mn <- (1-pars_vec$mdr_2000)*yinit3$E_mn
	yinit3$I_mm <- pars_vec$mdr_2000*yinit3$I_mn
	yinit3$I_mn <- (1-pars_vec$mdr_2000)*yinit3$I_mn
	...
	# Introducing PPQ resistance at the same time. Need to also consider resistance to both.
	ppq_init <- 0.01 # assuming 1%
	
	# Adding PPQ resistance to mosquitoes
	...
	# Adding PPQ resistance to humans
	...
	names(yinit3) <- ynames
		...
		### Running model for final time period	(2010-2020)
	possible_error <- tryCatch({	
		results <- as.data.frame(ode(y=yinit3, times=times, 
			func=malaria_model, pars_vec, method='lsoda')) # END NEW
	}, 
	error=function(e){cat("ERROR :",conditionMessage(e), "\n");e})
	...
	# PCR prevalence (not just population at risk)
		results$prev_pcr <- with(results,{(I_s+I_a)/560000}) 
	...
	monthly_east_llik <- calc_monthly_east_llik(results)	
	
	## malaria prevalence
	prev_east_llik <- calc_prev_east_llik(results)
	
	## Markers of resistance
	pfmdr_east_llik <- calc_pfmdr_east_llik(results)
	 pfpm2_east_llik <- calc_pfpm2_east_llik(results)
	
	## Phenotypic resistance
	mtes_east_llik <- calc_mtes_east_llik(results)
	ptes_east_llik <- calc_ptes_east_llik(results)
	
	# overall log likelihood
	  llik_east <-  monthly_east_llik   + prev_east_llik  +
			  pfmdr_east_llik + pfpm2_east_llik +
			 mtes_east_llik + ptes_east_llik 
	llik <- llik_east
...
prior <- function(pars) {
		...
		density <- density + dlnorm(pars[["N_init"]],  meanlog= 11.35, sdlog=0.4, log=T) # initial ppulation at risk -East
...
sample.prior <- function(n) {
	...
	mosq_m <- rlnorm(n, meanlog=1,sdlog=1)	
		N_init <- rlnorm(n, meanlog= 11.35, sdlog=0.4)	# east: mu= 11.35, sigma=0.6
...
write.csv(x=out$resample, file='east_pars_0_0.csv') # write CSV file with final parameter set to file
